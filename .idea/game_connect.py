...
class Connect(object):
    def send_to_all(lis,title,addr):#done
        sendRoomMessage(lis,title,addr)

    def dz_p1(s,title3,addr):#title = 'xszf_jdz' (显示字符)
        sendRoomMessage(s,title3,addr)#done
    ...
    def send_msg_all(s,title,users):#not
        for x in users:
            sendRoomMessage(s,title,x)



    def send_to_one(s,title,addr):#done
