    ...
    def jiaodizhu(dipai,users):  # 叫地主
        ...
        for x in lis:
            if x == 1:
                ...
                if a == 'y':
                    dizhu = 1
                    C.send_msg_all('玩家1叫地主','msg',users)
                else:
                    C.send_msg_all('玩家1不叫','msg',users)
            elif x == 2:
                a = C.dz_p2('是否要地主',title,users[1])
                if a == 'y':
                    dizhu = 2
                    C.send_msg_all('玩家2叫地主','msg',users)
                else:
                    C.send_msg_all('玩家2不叫','msg',users)
            elif x == 3:
                a = C.dz_p3('是否要地主',title,users[2])
                if a == 'y':
                    dizhu = 3
                    C.send_msg_all('玩家3叫地主','msg',users)
                else:
                    C.send_msg_all('玩家3不叫','msg',users)
        #根据地主位置生成出牌顺序
        if dizhu == 1:
            lt = [1, 2, 3]
        elif dizhu == 2:
        ...
        #给所有玩家发送地主玩家为谁
        title1 = 'xszf_num'
        C.send_msg_all("地主为玩家%d" % dizhu,title1,users)
        return dizhu, lt
